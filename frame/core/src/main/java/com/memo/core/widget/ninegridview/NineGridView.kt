...
class NineGridView @JvmOverloads constructor(
	...
	private val transferee by lazy { Transferee.getDefault(context) }

	private val mGridLayoutManager by lazy { NoScrollGridLayoutManager(context, 3) }
	...
	private fun addImage(images: List<String>) {
		...
		if (curImageSize + addImageSize < mMaxImageSize) {
			...
			// 去除加号 添加图片
			mAdapter.removeAt(curImageSize)
	...
	private fun removeAdd() {
		...
		if (mAdapter.data.size > 0 && mAdapter.data.last() == mAddDrawableRes) {
			mAdapter.removeAt(mAdapter.data.lastIndex)
		}
	}
