...
object PermissionHelper {
	 ...
	 */
	@JvmStatic
	fun grantedInstallUnKnowApp(activity: AppCompatActivity, requestCode: Int): Boolean {
			...
			.callback(object : PermissionUtils.FullCallback {
				...
				override fun onDenied(
					...
					if (permissionsDeniedForever.isNotEmpty()) {
						showOpenAppSettingDialog(activity, object : DialogHelper.Callback {
						...
						showNeedPermissionDialog(activity, object :
							DialogHelper.Callback {
							override fun onPositive() {
								requestStorageInSplash(activity, onGranted, onDenied)
							}
		...
		PermissionUtils.permission(*permissions)
			.callback(object : PermissionUtils.FullCallback {
				...
				override fun onDenied(
					...
					if (permissionsDeniedForever.isNotEmpty()) {
						showOpenAppSettingDialog(context, object : DialogHelper.Callback {
							override fun onPositive() {
								// 这里跳转到应用设置界面了
								AppUtils.launchAppDetailsSettings()
							}
						...
						showNeedPermissionDialog(context, object : DialogHelper.Callback {
							override fun onPositive() {
	...
	private fun showOpenAppSettingDialog(context: Context, callback: DialogHelper.Callback?) {
		...
		if (context is FragmentActivity) {
			...
			tipDialog.show(context.childFragmentManager)
		}
	}
	...
	@JvmStatic
	private fun showInstallRequestSettingDialog(activity: AppCompatActivity, requestCode: Int) {
		TipDialog(
			string(R.string.permission_title),
			string(R.string.permission_denied_forever_message)
		)
	...
	@JvmStatic
	private fun showNeedPermissionDialog(context: Context, callback: DialogHelper.Callback) {
		val tipDialog = TipDialog(
			string(R.string.permission_title),
