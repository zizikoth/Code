...
object NavigationHelper {
	...
	fun naviGaodeMap(toLat: Double, toLon: Double, toAddress: String) {
		if (AppHelper.isGaodeMapAvailable) {
			val uri =
				"amapuri://route/plan/?sourceApplication=${AppUtils.getAppName()}&dlat=$toLat&dlon=$toLon&dname=$toAddress&t=0"
	...
	fun naviBaiduMap(toLat: Double, toLon: Double, toAddress: String) {
		if (AppHelper.isBaiduMapAvailable) {
			val uri =
				"baidumap://map/direction?destination=name:$toAddress|latlng:$toLat,$toLon&mode=driving&sy=0&src=${AppUtils.getAppName()}"
			LogUtils.iTag("Navi", uri)
			val intent = Intent()
			intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
			intent.data = Uri.parse(uri)
	...
	fun naviTencentMap(toLat: Double, toLon: Double, toAddress: String) {
		if (AppHelper.isTencentMapAvailable) {
			val uri =
				"qqmap://map/routeplan?type=drive&from=当前位置&fromcoord=CurrentLocation&to=$toAddress&tocoord=$toLat,$toLon"
			LogUtils.iTag("Navi", uri)
			val intent = Intent()
			intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
			intent.data = Uri.parse(uri)
	...
	fun getGaodeWebUrl(toLat: Double, toLon: Double, toAddress: String): String {
		val url =
			"https://uri.amap.com/marker?position=$toLon,$toLat&name=$toAddress&src=${AppUtils.getAppName()}"
