...
object QRCodeHelper {
	...
	fun decodeQRCode(
		...
		lifecycleOwner.doInBackground(
			doInBackground = { QRCodeDecoder.syncDecodeQRCode(picPath) },
			onSuccess = { onSuccess.invoke(it) },
			onError = { onFailure.invoke(it) }
		)
	}
	...
	fun encodeQRCode(
		...
		lifecycleOwner.doInBackground(
			doInBackground = { QRCodeEncoder.syncEncodeQRCode(content, sizePx, foregroundColor, backgroundColor, logo) },
			onSuccess = { onSuccess.invoke(it) },
			onError = { onFailure.invoke(it) })
	}
}